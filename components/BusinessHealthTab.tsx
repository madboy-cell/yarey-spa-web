...
const BusinessHealthTab: React.FC<BusinessHealthTabProps> = ({ 
  ...
  const [pendingGoal, setPendingGoal] = useState(monthlyRevenueGoal);
  const [isSending, setIsSending] = useState(false);

  const t = TRANSLATIONS[language];

  useEffect(() => {
    setPendingGoal(monthlyRevenueGoal);
  }, [monthlyRevenueGoal]);
  ...
  const stats = useMemo(() => {
    ...
    // FIXED: Now includes both 'Paid' and 'Pending' bookings to capture all scheduled revenue
    ...
    // FIXED: Includes 'Pending' sessions in daily totals to show workload and expected income
  ...
  const sendLineReport = async () => {
    ...
    try {
      ...
      const topStaff = stats.staffLeaderboard.length > 0 && stats.staffLeaderboard[0].revenue > 0 
        ? stats.staffLeaderboard[0] 
        : null;
...
🏆 *DAILY HIGHLIGHTS*
${topStaff ? `🥇 Staff MVP: ${topStaff.name} (฿${topStaff.revenue.toLocaleString()})` : 'No therapist data for today'}
📦 Activity: ${stats.bookingCount} Sessions Scheduled` 
: 
...
🏆 *ไฮไลท์ประจำวัน*
${topStaff ? `🥇 พนักงานดีเด่น: ${topStaff.name} (฿${topStaff.revenue.toLocaleString()})` : 'ไม่มีข้อมูลพนักงานในวันนี้'}
📦 จำนวน: ${stats.bookingCount} รายการที่มีการจอง`;
      ...
      console.error(e);
      alert(language === 'en' ? '❌ Failed to send briefing. Check connection.' : '❌ ไม่สามารถส่งรายงานได้ กรุณาตรวจสอบการเชื่อมต่อ'); 
    } finally { 
      setIsSending(false); 
    }
  };
  ...
  const isGoalDirty = pendingGoal !== monthlyRevenueGoal;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button 
            ...
            disabled={isSending} 
            className={`px-6 py-3.5 rounded-2xl flex items-center gap-2 text-xs font-bold uppercase tracking-widest shadow-lg transition-all ${
              isSending ? 'bg-sage/50 cursor-not-allowed' : 'bg-sage hover:bg-sage-dark'
            } text-white`}
          >
          ...
          <div className="flex items-center gap-2 bg-white p-2 rounded-2xl border border-sage/10 shadow-sm">
            ...
            <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="bg-transparent font-bold text-sm text-charcoal p-2 cursor-pointer" />
              ...
              <div>
                ...
                <div className="flex items-center gap-2 text-[10px] text-sage font-bold uppercase tracking-widest">
                  ...
                  <div className="flex items-center gap-2">
                    <input type="number" value={pendingGoal} onChange={(e) => setPendingGoal(Number(e.target.value))} className="bg-sage/5 border-b border-sage/20 focus:border-sage outline-none px-1 w-24 text-[11px] font-bold text-charcoal" />
                    {isGoalDirty && (
                      <button onClick={() => onGoalUpdate(pendingGoal)} className="bg-sage text-white p-1 rounded-md text-[8px] px-2 flex items-center gap-1">
                        <Check size={8} /> {t.update}
                      </button>
                    )}
                  </div>
                </div>
