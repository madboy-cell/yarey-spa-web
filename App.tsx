...
const App: React.FC = () => {
  ...
  // Robust initialization for revenue goal
  const [monthlyRevenueGoal, setMonthlyRevenueGoal] = useState<number>(() => {
    const saved = localStorage.getItem('yarey_revenue_goal');
    if (saved) {
      const parsed = Number(saved);
      return !isNaN(parsed) && isFinite(parsed) ? parsed : 300000;
    }
    return 300000;
  });
  ...
  useEffect(() => {
    if (!isNaN(monthlyRevenueGoal) && isFinite(monthlyRevenueGoal)) {
    ...
    <div className="flex h-screen bg-cream overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col relative">
        <header className="h-20 bg-white border-b border-sage/10 flex items-center justify-between px-8 z-10 shrink-0">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2 bg-cream p-1 rounded-xl border border-sage/10">
              <button onClick={() => setLanguage('en')} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${language === 'en' ? 'bg-sage text-white shadow-sm' : 'text-sage/40 hover:text-sage'}`}>EN</button>
              <button onClick={() => setLanguage('th')} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${language === 'th' ? 'bg-sage text-white shadow-sm' : 'text-sage/40 hover:text-sage'}`}>ไทย</button>
            </div>
            <button 
        ...
        <div className="flex-1 relative overflow-hidden">
          {activeTab === 'dashboard' && <Dashboard bookings={bookings} staff={staff} services={services} language={language} onEditBooking={(b) => { setEditingBooking(b); setIsBookingModalOpen(true); }} onDeleteBooking={(b) => setConfirmDelete({ isOpen: true, title: language === 'en' ? "Cancel Appointment" : "ยกเลิกการนัดหมาย", message: language === 'en' ? `Are you sure?` : `คุณแน่ใจหรือไม่?`, onConfirm: () => deleteDoc(doc(db, 'bookings', b.id)) })} />}
          {activeTab === 'health' && <BusinessHealthTab bookings={bookings} services={services} staff={staff} language={language} serviceCosts={serviceCosts} inHouseHourlyRate={inHouseHourlyRate} outsourceHourlyRate={outsourceHourlyRate} monthlyRevenueGoal={monthlyRevenueGoal} onGoalUpdate={setMonthlyRevenueGoal} />}
          {activeTab === 'analytics' && <AnalyticsTab bookings={bookings} services={services} staff={staff} monthlyRevenueGoal={monthlyRevenueGoal} onGoalUpdate={setMonthlyRevenueGoal} />}
          {activeTab === 'revenue' && <RevenueTab bookings={bookings} services={services} language={language} />}
          {activeTab === 'cost' && <CostManagementTab bookings={bookings} services={services} staff={staff} language={language} inHouseHourlyRate={inHouseHourlyRate} outsourceHourlyRate={outsourceHourlyRate} onInHouseRateChange={setInHouseHourlyRate} onOutsourceRateChange={setOutsourceHourlyRate} serviceCosts={serviceCosts} onSaveCosts={setServiceCosts} />}
          {activeTab === 'services' && <ServicesTab services={services} language={language} onAdd={() => setIsServiceModalOpen(true)} onEdit={(s) => {setEditingService(s); setIsServiceModalOpen(true);}} onDelete={(s) => setConfirmDelete({ isOpen: true, title: "Delete", message: `Delete ${s.name}?`, onConfirm: () => deleteDoc(doc(db, 'services', s.id))})} />}
          {activeTab === 'staff' && <StaffTab staff={staff} language={language} onAdd={() => setIsStaffModalOpen(true)} onEdit={(m) => {setEditingStaff(m); setIsStaffModalOpen(true);}} onDelete={(m) => setConfirmDelete({ isOpen: true, title: "Delete", message: `Remove ${m.name}?`, onConfirm: () => deleteDoc(doc(db, 'staff', m.id))})} />}
        </div>
      </main>

      <BookingModal isOpen={isBookingModalOpen} onClose={() => setIsBookingModalOpen(false)} services={services} staff={staff} language={language} onSave={handleSaveBookings} bookingToEdit={editingBooking} />
